...
# set tracl header
sushw < DENISE_mod_y.su.shot$x.it1 > tmp2.su key=tracl a=1 b=1

# extract data 
suwind < tmp2.su > tmp.su key=tracl j=1
suwind < Profil_2/cachtice_data_FWI_Prof2_y.su.shot$x > tmp1.su key=tracl j=1
...
supswigp < tmp.su label1="Time [s]" label2="Trace No." title="Shot no. $x" fill=0 tracecolor=blue > true_$x.ps
supswigp < tmp1.su label1="Time [s]" label2="Trace No." title="Shot no. $x" fill=0 tracecolor=red > mod_$x.ps
