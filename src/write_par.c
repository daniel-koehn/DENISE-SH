...
void write_par(FILE *fp){
	...
	extern int HESSIAN, GRAD_METHOD, ORDER_HESSIAN, STEPMAX, TRKILL;
	...
	if (INVMAT==10){
		...
		if (INVMAT==0){fprintf(fp," INVMAT=%d: Time Domain - FWI is applied.\n",INVMAT);}
                if (INVMAT==1){fprintf(fp," INVMAT=%d: Frequency Domain - FWI is applied.\n",INVMAT);}
	}
